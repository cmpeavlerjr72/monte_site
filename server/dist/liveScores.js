...
const PORTAL_FRIENDS = new Map();
{
    ...
    if (PORTAL_FRIENDS.size) {
        console.log("[portal] friend pairs:", [...PORTAL_FRIENDS.entries()].map(([k, v]) => `${k}<->${v.join("+")}`).join(", "));
    }
}
...
const PORTAL_OWNER_UIDS = new Map();
{
...
if (supa) {
    console.log("[accounts] Supabase configured; portal owners:", PORTAL_OWNER_UIDS.size
...
const supaTokenCache = new Map();
async function supaVerify(jwt) {
    if (!supa || !jwt)
        return null;
    const k = crypto.createHash("sha256").update(jwt).digest("hex");
    const hit = supaTokenCache.get(k);
    if (hit && Date.now() - hit.at < SUPA_TOKEN_TTL_MS)
        return hit.user;
    let user = null;
    try {
    ...
    if (supaTokenCache.size >= SUPA_TOKEN_CACHE_MAX)
        supaTokenCache.clear();
 ...
 */
async function supabaseAuth(req, _res, next) {
    if (!supa) {
        next();
        return;
    }
    ...
    if (m) {
        try {
            req.supaUser = await supaVerify(m[1].trim());
        }
...
async function portalPayloadFor(acct) {
 ...
 * no new timer either.
 */
async function portalSettlementsFor(acct) {
 ...
 */
app.get("/api/portfolio/cfb/friends", asyncRoute(async (req, res) => {
    const acct = portalGate(req, res);
    if (!acct)
        return;
    const friends = [];
    ...
    if ((resp.status === 200 || resp.status === 201) && orderId) {
        ...
        try {
            ...
            for (const wait of [0, 250, 600, 1200]) {
                ...
                if (terminal || (w.mode === "take" && !tifDowngraded && rem === 0) || w.mode === "rest")
                    break;
            }
            state = {
    ...
    try {
        for (const acct of PORTAL_ACCOUNTS) {
            ...
            const listening = pushSubs.some((s) => s.account === acct.id) ||
                (PORTAL_FRIENDS.get(acct.id) ?? []).some((fid) => pushSubs.some((s) => s.account === fid));
            if (!listening)
