...
function espnUrl(sport, dateYYYYMMDD, opts) {
    ...
    // TEST-ONLY SEAM, this file only (never committed, never shipped): redirect
    // the CFB scoreboard fetch at a local fake-ESPN server so the negative
    // cache's real 403-detection wiring (fetchGroupPages -> fetchJsonWithTimeout
    // -> err.status) can be exercised end-to-end without touching prod code.
    const base = process.env.TEST_ESPN_BASE || "https://site.api.espn.com/apis/site/v2/sports/football/college-football";
    return `${base}/scoreboard?dates=${d}&groups=${g}&limit=${limit}`;
}
...
// TEST-ONLY EXPORTS, this file only (never committed): let the verification
// script assert the negative-cache state machine directly instead of only
// observing it through HTTP timing.
export { server, espnBlockActive, noteEspn403, noteEspnReachable };
