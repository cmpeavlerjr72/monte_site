...
let marketSignerAcct: PortalAccount | null = null;
...
// ============================================================================
// My-Kalshi portfolio portal (password-gated, PASSWORD = ACCOUNT)
//
// Resting orders + fills on the NCAAF market families, so the scoreboard can
// pin and badge the games the logged-in trader has money on. This deliberately
// amends the older "no credentials anywhere in this repo" stance (user
// decision 2026-08-26): credentials are still never in CODE — they arrive only
// through deployment env vars.
//
// MULTI-ACCOUNT (user decision 2026-08-31, "get Roth trading"): the portal
// serves MORE THAN ONE Kalshi account, and the LOGIN PASSWORD IS the account
// selector — each password maps to exactly one account, and a session only
// ever sees and trades the account its password names. The registry mirrors
// kalshi-rfq's config/accounts.json env_suffix convention:
//
//   base account (the owner, suffix ""):
//     CFB_PORTAL_PASSWORD       login password  (CFB_PORTFOLIO_TOKEN = alias)
...
//     CFB_ORDERS_LIVE           "1" = order entry live (else dry-run staged)
//
...
//
// No accounts configured means the route family answers 503, never a stack
// trace and never an open portal. The token check is timing-safe and runs
// against EVERY registered password on every attempt (no early exit), so
// response time does not say which passwords exist. This route family is
// where order entry lives, so the auth gate sits BEFORE every mutating
// endpoint.
// ============================================================================

/** One Kalshi account the portal can serve. `key` is the lazy-loaded private
 *  key: undefined = not tried yet, null = tried and unavailable. */
type PortalAccount = {
  id: string;         // "mp" for the base account, else the env suffix, lowercased
  label: string;      // client-facing name, shown in the My Book header
  password: string;   // this account's portal login
  keyId: string;
  pemInline: string;
  pemPath: string;
...
function portalBuildAccounts(): PortalAccount[] {
  ...
  for (const k of Object.keys(process.env).sort()) {
    ...
    if (out.some((a) => a.password === acct.password)) {
      console.error(
        `[portal] account ${acct.label} DROPPED: its password duplicates an ` +
...
if (supa) {
  console.log("[link] user Kalshi linking:",
    LINK_ENABLED
      ? `enabled (${process.env.CFB_ORDERS_LIVE_USERS === "1" ? "orders LIVE" : "staged"})`
      : "disabled (no KALSHI_CRED_SECRET)");
}

type CredRow = { key_id: string; pem_ct: string; iv: string; tag: string; created_at?: string };

function credEncrypt(plain: string): { pem_ct: string; iv: string; tag: string } {
...
function credDecrypt(row: CredRow): string {
...
const maskKeyId = (k: string) => (k.length <= 4 ? "…" : "…" + k.slice(-4));
...
const linkedCache = new Map<string, { at: number; acct: PortalAccount | "unreadable" | null }>();
const linkedCacheDrop = (uid: string) => { linkedCache.delete(uid); };

async function linkedAccountFor(uid: string): Promise<PortalAccount | "unreadable" | null> {
  if (!LINK_ENABLED || !uid) return null;
  const hit = linkedCache.get(uid);
  if (hit && Date.now() - hit.at < LINK_CACHE_TTL_MS) return hit.acct;

  let acct: PortalAccount | "unreadable" | null = null;
  try {
    ...
    const row = data as CredRow | null;
    if (row) {
      ...
      if (!key) {
        ...
        try {
          ...
          label = String((prof as any)?.handle || "");
 ...
 */
async function portalLinkedUser(req: Request, _res: Response, next: NextFunction) {
  if (!LINK_ENABLED) { next(); return; }
  const uid = supaUserOf(req)?.id;
  if (uid && !PORTAL_OWNER_UIDS.has(uid)) {
    try { (req as any).portalLinked = await linkedAccountFor(uid); }
    catch { /* never block a request on the accounts layer */ }
  }
  next();
}
...
function portalLinkedOf(req: Request): PortalAccount | "unreadable" | null {
  return ((req as any).portalLinked as PortalAccount | "unreadable" | null) ?? null;
}
...
function linkGate(req: Request, res: Response): SupaUser | null {
  res.set("Cache-Control", "no-store");
  if (!LINK_ENABLED) {
  ...
  if (!user) { res.status(401).json({ error: "sign_in_required" }); return null; }
  return user;
}

app.get("/api/me/kalshi", asyncRoute(async (req: Request, res: Response) => {
  ...
  const { data, error } = await supa!
    ...
    .eq("user_id", user.id)
    .maybeSingle();
  if (error) { res.status(500).json({ error: "lookup_failed" }); return; }
  const row = data as { key_id: string; created_at: string } | null;
  res.json({
    linked: Boolean(row),
...
app.post("/api/me/kalshi", asyncRoute(async (req: Request, res: Response) => {
  const user = linkGate(req, res);
  if (!user) return;
  ...
  if (len > 64_000) { res.status(413).json({ error: "too_large" }); return; }
  ...
  if (!keyId || keyId.length > 120) {
    res.status(400).json({ error: "bad_key_id" }); return;
  }
  if (!pem || pem.length > LINK_MAX_PEM || !pem.includes("BEGIN")) {
    res.status(400).json({ error: "bad_pem" }); return;
  }
  ...
  if (tErr) console.warn("[link] is_trader update failed:", tErr.message);
...
app.delete("/api/me/kalshi", asyncRoute(async (req: Request, res: Response) => {
  const user = linkGate(req, res);
  if (!user) return;
  const { error } = await supa!
    .from("kalshi_credentials").delete().eq("user_id", user.id);
  if (error) { res.status(500).json({ error: "unlink_failed" }); return; }
  const { error: tErr } = await supa!
...
async function portalMveInfo(acct: PortalAccount, ticker: string) {
...
function portalPrivateKey(acct: PortalAccount): crypto.KeyObject | null {
  if (acct.key !== undefined) return acct.key;
  try {
...
async function portalGet(acct: PortalAccount, apiPath: string): Promise<any> {
  const key = portalPrivateKey(acct);
  if (!key || !acct.keyId) throw new Error("portal_credentials_missing");
  for (let attempt = 0; ; attempt++) {
    const ts = String(Date.now());
...
type PortalPayload = {
  ...
  /** Order-entry staging state for THIS account (CFB_ORDERS_LIVE[_X]).
   *  Forwarded so the confirm popup can wear its DRY RUN badge BEFORE the
   *  user presses Confirm, rather than discovering the staging only in the
   *  response. */
  orders_live: boolean;
  /** Which account this book IS — the password picked it. The label renders
   *  in the My Book header so a trader always knows whose money is on
   *  screen. Absent on a server that predates multi-account. */
  account_id?: string;
  account_label?: string;
};

async function portalOrders(acct: PortalAccount): Promise<PortalOrder[]> {
...
async function portalFills(acct: PortalAccount): Promise<PortalFill[]> {
...
async function portalPositions(acct: PortalAccount): Promise<PortalPosition[]> {
 ...
 */
async function portalSettlements(acct: PortalAccount): Promise<PortalSettlement[]> {
...
const portalCaches = new Map<string, { at: number; payload: PortalPayload }>();
...
const portalSettleCaches = new Map<string, { at: number; payload: { fetched_at: string; settlements: PortalSettlement[] } }>();
 ...
 */
function portalGate(req: Request, res: Response): PortalAccount | null {
  ...
  let hit: PortalAccount | null = null;
...
function ordersAudit(acct: PortalAccount, rec: Record<string, unknown>): void {
 ...
 */
const unitSizeCache = new Map<string, { at: number; unit: number | null }>();
const UNIT_CACHE_TTL_MS = 10 * 60_000;

async function userUnitSize(uid: string): Promise<number | null> {
  if (!supa || !uid) return null;
  const hit = unitSizeCache.get(uid);
  if (hit && Date.now() - hit.at < UNIT_CACHE_TTL_MS) return hit.unit;
  let unit: number | null = null;
  try {
    const { data, error } = await supa
      .from("profiles").select("unit_size").eq("id", uid).maybeSingle();
    if (error) throw new Error(error.message);
    const v = Number((data as any)?.unit_size);
...
const appTickerCaches = new Map<string, { at: number; tickers: Set<string> | null }>();
 ...
 */
function appPlacedTickers(acct: PortalAccount): Set<string> | null {
  ...
  try {
    ...
    for (const line of fs.readFileSync(ORDERS_AUDIT_PATH, "utf8").split("\n")) {
      ...
      if (String(rec.account || "mp") !== acct.id) continue;
...
type SpendEntry = { at: number; cost: number; account: string };
...
function ordersSpent24h(acct: PortalAccount): number {
...
const ordersIdemScope = (acct: PortalAccount, key: string) => `${acct.id}:${key}`;
...
async function portalSend(
  acct: PortalAccount, method: "POST" | "DELETE", apiPath: string, body?: unknown,
...
type OrdersBook = {
  ...
  /** Contracts resting at the best level of each side, when the book reports
   *  them. The ASK sizes are what the partial-fill "continue at the next
   *  price?" offer prints — how much is actually there to take. */
...
async function ordersBook(ticker: string): Promise<OrdersBook> {
  ...
  const best = (levels: any): { px: number; size: number | null } | null => {
    let out: { px: number; size: number | null } | null = null;
...
async function ordersReadState(acct: PortalAccount, orderId: string): Promise<
...
async function ordersRestingApp(acct: PortalAccount): Promise<AppRestingOrder[]> {
...
app.post("/api/portfolio/cfb/orders/cancel", asyncRoute(async (req: Request, res: Response) => {
  ...
  if (!all && cancelled.length === 1) {
    (cancelled[0] as any).state = await ordersReadState(acct, cancelled[0].order_id);
  }
 ...
 *  another's. */
async function ordersOwnedByApp(acct: PortalAccount, orderId: string): Promise<boolean> {
  try {
    const back = await portalGet(acct, `/portfolio/orders/${encodeURIComponent(orderId)}`);
    const ord = back?.order || back || {};
...
app.post("/api/portfolio/cfb/orders/convert", asyncRoute(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const del = await portalSend(
      acct, "DELETE", `/portfolio/events/orders/${encodeURIComponent(orderId)}`);
...
/** `account` scopes the alerts: a device hears about the account it was
 *  logged into when it subscribed, and nothing else. Re-subscribing after a
 *  login to the other account MOVES the device (replace-by-endpoint below) —
 *  alerts follow the most recent login on that device. */
type PushSub = { endpoint: string; keys: { p256dh: string; auth: string }; account: string };
...
try {
  ...
  if (Array.isArray(raw)) {
    pushSubs = raw.map((s: any) => ({ ...s, account: String(s?.account || "mp") }));
  }
 ...
 *  lesson 2026-08-29). */
async function pushSendAll(accountId: string, payload: Record<string, unknown>): Promise<{
...
app.post("/api/push/test", asyncRoute(async (req: Request, res: Response) => {
  ...
  if (!devices) { res.status(400).json({ error: "no_subscriptions" }); return; }
...
const pushWatermarks = new Map<string, { fill: number; settle: number }>();
function pushWatermarkOf(accountId: string): { fill: number; settle: number } {
  let w = pushWatermarks.get(accountId);
  if (!w) { w = { fill: Date.now(), settle: Date.now() }; pushWatermarks.set(accountId, w); }
  return w;
}
...
async function pushPollAccount(acct: PortalAccount): Promise<void> {
  const mark = pushWatermarkOf(acct.id);
  try {
    const fills = await portalFills(acct);
    const fresh = fills.filter((f) =>
      !f.is_taker && Date.parse(f.created_time) > mark.fill);
    ...
    for (const f of fills) {
      ...
      if (Number.isFinite(ts) && ts > mark.fill) mark.fill = ts;
    }
  } catch (err: any) {
  ...
  try {
    ...
    const fresh = settlements.filter((s) =>
      Date.parse(s.settled_time) > mark.settle);
    ...
    for (const s of settlements) {
      ...
      if (Number.isFinite(ts) && ts > mark.settle) mark.settle = ts;
    }
  } catch (err: any) {
