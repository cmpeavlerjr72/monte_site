...
function rowsForSliceGeneric<T extends {
    teamA: string; teamB: string; confA?: string; confB?: string;
    isFavoritePick: boolean; confidence: number; weekNum: number;
  }>(rows: T[], s: TrendSlice, confOf?: (team?: string) => string | undefined): T[] {
    return rows.filter((r: T) => {
      ...
      // pick type
      if (s.pickType !== "all") {
        if (s.pickType === "favorite" && !r.isFavoritePick) return false;
        if (s.pickType === "underdog" &&  r.isFavoritePick) return false;
      }
  
      // ✅ fallback to team_info mapping when CSV lacks conferences
      ...
      if (s.conference === "all") {
        ...
        if (s.gameType === "conference") return sameConf && a === cf && b === cf;
...
type UpcomingPick = {
    ...
    pickText: string;           // e.g., "Ole Miss +3.5"
};
...
function useCLVRows() {
    ...
    const [rowsUpcoming, setRowsUpcoming] = useState<UpcomingPick[]>([]);
          ...
          for (const w of Object.keys(simsByWeek)) {
            ...
            for (const arr of metaArrays) {
              for (const row of arr as any[]) {
                ...
                // prefer Team A/B, else Home/Away (same as your CLV logic)
                ...
                let teamA = "", teamB = "", homeTeam = "", awayTeam = "";
                if (csvTeamA && csvTeamB) {
                  ...
                  if (csvHome && String(csvHome).trim().toLowerCase() === teamA.toLowerCase()) {
                    ...
                    homeTeam = String(csvHome ?? ""); awayTeam = String(csvAway ?? "");
                  }
                } else if (csvHome && csvAway) {
                  teamA = String(csvHome).trim(); teamB = String(csvAway).trim();
                  homeTeam = teamA; awayTeam = teamB;
                } else continue;
                ...
                let confA = pick<string>(row, ["Team A Conf","team_a_conf","confA","ConfA","A Conf","home_conf"]) ?? undefined;
                let confB = pick<string>(row, ["Team B Conf","team_b_conf","confB","ConfB","B Conf","away_conf"]) ?? undefined;

                // ✅ add this (same as Results.tsx behavior)
                ...
                if (!Number.isFinite(openHome)) continue;
  
                const rowsAlpha: SimRow[] = (sim.teamA === teamA)
                  ? sim.rowsA
                  : sim.rowsA.map(r => ({ team: teamA, opp: teamB, pts: r.opp_pts, opp_pts: r.pts }));
  
                // convert book home line → Team A orientation
                let openingSpreadA: number, closingSpreadA: number | undefined;
                if (csvHome && csvAway && homeTeam) {
                  const teamAIsHome = teamA.toLowerCase() === homeTeam.toLowerCase();
                  openingSpreadA = teamAIsHome ? (openHome as number) : -(openHome as number);
                  closingSpreadA = Number.isFinite(closeHome)
                    ? (teamAIsHome ? (closeHome as number) : -(closeHome as number))
                    : undefined;
                } else {
                  openingSpreadA = openHome as number;
                  closingSpreadA = Number.isFinite(closeHome) ? (closeHome as number) : undefined;
                }
                ...
                const confidence = pickSide === "A" ? pA_open : pB_open;
                const isFavoritePick = (openingSpreadA < 0 && pickSide === "A") || (openingSpreadA > 0 && pickSide === "B");
  
                const kickoffMs = kickoffMsFrom(row);
                const weekNum = parseInt(w.replace(/[^0-9]/g, "") || "0", 10);
  
                if (typeof closingSpreadA === "number") {
                  // Completed game → CLV row
                  ...
                  out.push({
                    week: w, weekNum, kickoffMs,
                    teamA, teamB, confA, confB,
                    pickSide, isFavoritePick, confidence, clvProb,
                  });
                } else {
                  // Upcoming pick (no closing spread yet)
                  const pickText =
                    pickSide === "A"
                      ? `${teamA} ${betLine > 0 ? `+${betLine}` : `${betLine}`}`
                      : `${teamB} ${(-betLine) > 0 ? `+${-betLine}` : `${-betLine}`}`;
  
                  upcoming.push({
                    week: w, weekNum, kickoffMs,
                    key: `${key}__spread`,
                    teamA, teamB, confA, confB,
                    pickSide, isFavoritePick, confidence,
                    pickText,
                  });
                }
              }
    ...
    return { loading, rows, rowsUpcoming, weeksAvailable };
  }
...
export default function TrendsCLV() {
  const { loading, rows, rowsUpcoming, weeksAvailable } = useCLVRows();
  ...
  const [weekSel, setWeekSel] = useState<number | "">("");
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: 16 }}>
      ...
      {/* --- Suggested bets from top CLV trends (Spread) --- */}
        <section className="card" style={{ padding: 12, marginTop: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <h3 style={{ margin: 0, fontWeight: 800, fontSize: 18 }}>Suggested spread bets (by top CLV trends)</h3>
            <label style={{ fontSize: 13, color: "var(--muted)" }}>Week:</label>
            <select
            value={weekSel ?? ""}
            onChange={(e) => setWeekSel(e.target.value ? Number(e.target.value) : "")}
            style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--card)", fontWeight: 600 }}
            >
        ...
        {weekSel === "" ? (
            <div style={{ padding: 10, opacity: 0.75 }}>Choose a week to show picks.</div>
        ) : (
            (() => {
            const wk = Number(weekSel);
            const upcomingThisWeek = rowsUpcoming.filter(r => r.weekNum === wk);
            const TOP_SLICE_COUNT = 30;
            const topSlices = scored.slice(0, TOP_SLICE_COUNT);

            const matches: { pick: UpcomingPick; trend: TrendScored }[] = [];
            for (const pick of upcomingThisWeek) {
                let best: TrendScored | null = null;
                for (const t of topSlices) {
                  if (rowsForSliceGeneric([pick], t.slice, confOf).length) {  // ← pass confOf
                    if (!best || t.score > best.score) best = t;
                  }
                }
                if (best) matches.push({ pick, trend: best });
            }
              

            // de-dupe by game+pick; keep highest score
            const byKey = new Map<string, { pick: UpcomingPick; trend: TrendScored }>();
            for (const m of matches) {
                const k = `${m.pick.teamA}__${m.pick.teamB}__${m.pick.pickText}`;
                const prev = byKey.get(k);
                if (!prev || m.trend.score > prev.trend.score) byKey.set(k, m);
            }
            const list = Array.from(byKey.values())
                .sort((a,b)=> (b.trend.score - a.trend.score) || (b.pick.confidence - a.pick.confidence))
                .slice(0, 12);
            ...
            return list.length ? (
                ...
                <div
                    style={{
                    display: "grid",
                    gridTemplateColumns: "40px 1fr 110px 140px 1fr",
                    gap: 8, fontSize: 13, fontWeight: 700, color: "var(--muted)",
                    padding: "6px 8px",
                    }}
                >
                    <div>#</div><div>Game / Pick</div><div>Confidence</div><div>CLV Trend Score</div><div>Trend</div>
                </div>
                {list.map((e, i) => (
                    <div
                    key={`${e.pick.key}-${i}`}
                    ...
                    <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 800 }}>{e.pick.teamA} vs {e.pick.teamB}</div>
                        <div style={{ fontSize: 12, color: "var(--muted)" }}>
                        {e.pick.pickText}{e.pick.kickoffMs ? ` • ${new Date(e.pick.kickoffMs).toLocaleString()}` : ""}
                        </div>
                    </div>
                    <div>{(e.pick.confidence * 100).toFixed(1)}%</div>
                    <div style={{ fontWeight: 800, color: e.trend.score >= 0 ? "#166534" : "#991b1b" }}>
                        {e.trend.score.toFixed(2)}
                    </div>
                    <div style={{ fontSize: 12 }}>{sliceLabel(e.trend.slice)}</div>
                    </div>
                ))}
                </div>
            ) : (
                <div style={{ padding: 12, opacity: 0.75 }}>
                No matches for the selected week under the top CLV trend slices.
                </div>
            );
            })()
        )}
